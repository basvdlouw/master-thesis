...
% For a more complete picture, we graphed different scenarios to visualize the difference in entropy, depending on the parameters. 
% As the average number of open ports will slightly increase when more ports are scanned, we used the formula \(K * \log_{10}(N)\) to calculate this probability, where N is the number of ports scanned, and K is a variable that we can use to measure the difference between a higher and lower number of open ports.
% We graphed K = 1.5, 2, 2.5 and 3. Reference table~\ref{tab:ref-table-avg-open} showcases the average number of open ports based on the number of port scans.
...
%       \toprule
%       Ports scanned & K & Average number of open ports  \\
%        \midrule
%       1000 & 1.5 & 
%       10000 & 1.5 &
%       1000 & 2 &
...
%        \bottomrule
%   \end{tabular}
%   \caption{Average number of open ports based on \(K * \log_{10}(N)\) formula}
%   \label{tab:ref-table-avg-open}
% \end{table}
...
We have integrated browser-based port scanning~\citeartifact{fpjspr} within the popular FingerprintJS~\citeartifact{fingerprintjs} library, and we assessed that 1,000 ports can easily be scanned on any webpage, together with existing fingerprinting techniques, without noticeable impact on the end user. 
